...
const features = {
  ...
  performance: {
    ...
    body: 'See how your workload, minutes, speed, and recovery have changed across recent matches. The goal is better decisions, not more pressure.',
  },
  highlights: {
    eyebrow: 'Your story',
    ...
    <main className="app-stage">
      <div className="stage-toolbar">
        ...
        <div className="demo-controls">
          ...
          {!isMinistry && !isFederation && !isCoach && !isMedical && <><button className="role-switch" onClick={() => { setView('ministry'); setMinistrySection('overview'); setMobileShell(false) }}>Open Ministry view</button><button className="role-switch" onClick={() => { setView('federation'); setFederationSection('overview'); setMobileShell(false) }}>Open FERWAFA view</button><button className="role-switch" onClick={() => { setView('coach'); setCoachSection('overview'); setMobileShell(false) }}>Open Coach view</button><button className="role-switch" onClick={() => { setView('medical'); setMedicalSection('overview'); setMobileShell(false) }}>Open Medical view</button></>}
          {(isMinistry || isFederation || isCoach || isMedical) && <button className="role-switch" onClick={() => { setView('home'); setMobileShell(true) }}>Return to Athlete</button>}
          ...
          <div className="app-layout">
            <aside className="sidebar">
              <div className="side-intro">
                {isMinistry ? <><span className="side-overline">MINISTRY VIEW</span><strong>National system</strong><span>Rwanda · 05 Aug 2026</span></> : isFederation ? <><span className="side-overline">FERWAFA VIEW</span><strong>Football system</strong><span>Rwanda · 05 Aug 2026</span></> : isCoach ? <><span className="side-overline">COACH VIEW</span><strong>{athlete.club} {athlete.competitionLevel}</strong><span>Performance staff</span></> : isMedical ? <><span className="side-overline">MEDICAL STAFF</span><strong>Clinical workspace</strong><span>AS Kigali · U20</span></> : <><span className="side-overline">ATHLETE PORTAL</span><strong>{athlete.name}</strong><span>{athlete.club} · {athlete.competitionLevel}</span></>}
              </div>
              <nav aria-label="Athlete navigation">
                {(isMinistry ? ministryNavItems : isFederation ? federationNavItems : isCoach ? coachNavItems : isMedical ? medicalNavItems : navItems).map(({ id, label, icon: Icon, target }, index) => (
                  <button key={`${isMinistry ? 'ministry' : isFederation ? 'federation' : isCoach ? 'coach' : isMedical ? 'medical' : 'athlete'}-${label}-${index}`} className={`nav-item ${isMinistry ? (ministrySection === (target?.replace('ministry-', '') as MinistrySection) ? 'is-current' : '') : isFederation ? (federationSection === (target?.replace('federation-', '') as FederationSection) ? 'is-current' : '') : isCoach ? (coachSection === (target?.replace('coach-', '') as CoachSection) ? 'is-current' : '') : isMedical ? (medicalSection === (target?.replace('medical-', '') as MedicalSection) ? 'is-current' : '') : (view === id ? 'is-current' : '')}`} onClick={() => isMinistry ? selectMinistrySection(target) : isFederation ? selectFederationSection(target) : isCoach ? selectCoachSection(target) : isMedical ? selectMedicalSection(target) : go(id)}>
            ...
            <section className="content-area">
              ...
              <nav className="bottom-nav" aria-label="Mobile navigation">
                {(isMinistry ? ministryNavItems : isFederation ? federationNavItems : isCoach ? coachNavItems : isMedical ? medicalNavItems : navItems).map(({ id, label, icon: Icon, target }) => (
                  <button key={`${isMinistry ? 'ministry' : isFederation ? 'federation' : isCoach ? 'coach' : isMedical ? 'medical' : 'athlete'}-${label}`} className={`bottom-item ${isMinistry ? (ministrySection === (target?.replace('ministry-', '') as MinistrySection) ? 'is-current' : '') : isFederation ? (federationSection === (target?.replace('federation-', '') as FederationSection) ? 'is-current' : '') : isCoach ? (coachSection === (target?.replace('coach-', '') as CoachSection) ? 'is-current' : '') : isMedical ? (medicalSection === (target?.replace('medical-', '') as MedicalSection) ? 'is-current' : '') : (view === id ? 'is-current' : '')}`} onClick={() => isMinistry ? selectMinistrySection(target) : isFederation ? selectFederationSection(target) : isCoach ? selectCoachSection(target) : isMedical ? selectMedicalSection(target) : go(id)}>
...
function HomeView({ onFeature, onNavigate }: { onFeature: (feature: keyof typeof features) => void; onNavigate: (view: View) => void }) {
  return <>
    <div className="welcome-row"><div><span className="eyebrow">TUESDAY · 05 AUGUST 2026</span><h1>Good morning, {athlete.name.replace(' M.', '')}.</h1><p className="lede">Your next step is clear. Keep your load steady and check in after training.</p></div><div className="weather"><CloudSun size={25} /><span>24°<small>Kigali</small></span></div></div>
    <div className="readiness-card">
      <div><span className="card-label">TODAY'S READINESS</span><div className="readiness-score">{athlete.readiness}<span>/100</span></div><p className="positive-copy"><Check size={15} weight="bold" /> Ready to train</p></div>
      <div className="readiness-ring"><div><strong>{athlete.readiness}</strong><span>ready</span></div></div>
      <button className="text-button" onClick={() => onFeature('health')}>See details <ArrowUpRight size={16} /></button>
    </div>
...
function PassportView({ onInfo }: { onInfo: () => void }) {
  return <><PageHeader eyebrow="YOUR RECORD" title="Digital Sports Passport" description="The record that travels with you, from U20 to the national team." onInfo={onInfo} /><div className="passport-card"><div className="passport-top"><div><span className="card-label">ATHLETIC RWANDA ID</span><strong>{athlete.id}</strong></div><ShieldCheck size={28} weight="fill" /></div><div className="passport-person"><div className="avatar">EM</div><div><h2>{athlete.name}</h2><p>{athlete.position} · {athlete.club} · {athlete.competitionLevel}</p></div><span className="verified-pill"><Check size={13} /> Verified</span></div><div className="passport-grid"><DataPoint label="Eligibility" value={athlete.eligibility} /><DataPoint label="Medical status" value={athlete.medicalStatus} tone="warning" /><DataPoint label="Current season" value="2026" /><DataPoint label="Next appointment" value={athlete.nextAppointment} /></div></div><div className="section-heading compact"><div><span className="eyebrow">PASSPORT TRAIL</span><h2>Recent updates</h2></div></div><div className="timeline"><TimelineItem date="Today · 08:40" title="Wellness check completed" detail={`Readiness score calculated at ${athlete.readiness}/100.`} icon={<Heartbeat />} /><TimelineItem date="Yesterday · 17:20" title="Match record added" detail={`${athlete.club} vs. Rayon Sports · ${athlete.minutesPlayed} minutes.`} icon={<Activity />} /><TimelineItem date="12 May 2026" title="Eligibility verified" detail="Federation record confirmed for the current competition." icon={<ShieldCheck />} /></div></>
}

function PerformanceView({ onInfo, onHighlights }: { onInfo: () => void; onHighlights: () => void }) {
  return <><PageHeader eyebrow="YOUR PROGRESS" title="Performance history" description="A clearer picture of your match rhythm and recovery." onInfo={onInfo} /><div className="performance-hero"><div><span className="card-label">CURRENT FORM</span><h2>Steady and improving</h2><p>Based on your last five appearances.</p></div><div className="form-score">8.2<small>/10</small></div></div><div className="metric-grid"><Metric label="Minutes" value="384" change="+12%" /><Metric label="Top speed" value="28.4" unit="km/h" change="+1.8" /><Metric label="Recovery" value="82" unit="%" change="Good" /></div><div className="chart-panel"><div className="panel-title"><div><span className="eyebrow">LOAD / RECOVERY</span><h2>Last 5 matches</h2></div><button className="icon-button" onClick={onInfo} aria-label="Explain load and recovery"><Info size={18} /></button></div><LargeChart /></div><div className="video-frame"><div className="video-copy"><span className="eyebrow">MATCH MOMENT</span><h2>AS Kigali vs Rayon Sports</h2><p>90 minutes · Right-back · 8 selected moments</p></div><video controls preload="metadata" poster="/performance-poster.svg"><source src="/performance-demo.mp4" type="video/mp4" />Your browser does not support video playback.</video></div><button className="highlight-card" onClick={onHighlights}><span className="video-icon"><PlayCircle size={25} weight="fill" /></span><span><strong>Video highlight reel</strong><small>8 selected moments · Updated yesterday</small></span><CaretRight size={19} /></button></>
}
...
function LargeChart() { return <svg className="large-chart" viewBox="0 0 720 220" role="img" aria-label="Load and recovery chart"><g className="chart-grid"><path d="M0 40H720M0 95H720M0 150H720M0 205H720" /></g><path d="M0 145 C45 128 62 152 110 112 S180 126 224 92 S280 100 330 117 S390 72 430 87 S490 81 535 54 S595 82 645 48 S690 52 720 32" fill="none" stroke="#b7f36b" strokeWidth="4" strokeLinecap="round" /><path d="M0 168 C50 150 73 170 120 152 S185 168 230 141 S291 146 340 158 S395 121 440 133 S505 120 550 105 S605 127 650 105 S700 110 720 94" fill="none" stroke="#8f8ba8" strokeWidth="3" strokeDasharray="7 8" strokeLinecap="round" /><g className="chart-labels"><text x="0" y="219">APR 20</text><text x="160" y="219">MAY 03</text><text x="320" y="219">MAY 10</text><text x="480" y="219">MAY 17</text><text x="650" y="219">TODAY</text></g></svg> }
